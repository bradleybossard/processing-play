...
void draw() {
  ...
  // In debug, we just loop forever.  If not, create frames for a gif.
  if (debug) {
    return;
  }
  
  // gfisicle command line
  // gifsicle --delay=4 --optimize --loopcount blob-* > blob.gif
  saveFrame("blob-#####.gif");
  
  if (abs(phaseAngle) >= 360) {
    noLoop();
  }
}
